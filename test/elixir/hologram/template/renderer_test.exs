defmodule Hologram.Template.RendererTest do
  ...
  describe "inferred cid (key/1)" do
    test "a component defining key/1 is stateful without a cid prop at the call site" do
      node = {:component, Module86, [{"row", [expression: {%{id: "abc"}}]}], []}

      assert render_dom(node, @env, @server) ==
               {"<div>row = abc</div>",
                %{"abc" => %{module: Module86, struct: %Component{state: %{}}}}, @server}
    end

    test "the inferred cid is scoped by the enclosing stateful component" do
      node = {:component, Module87, [{"cid", [text: "parent"]}], []}
    ...
    test "the same key under two different parents resolves to two distinct components" do
      node =
        {:element, "div", [],
         [
           {:component, Module87, [{"cid", [text: "list-1"]}], []},
           {:component, Module87, [{"cid", [text: "list-2"]}], []}
         ]}
    ...
    test "an explicit cid prop wins over key/1 and stays unscoped" do
      ...
      assert render_dom(node, @env, @server) ==
               {"<div>row = a</div>",
                %{"spelled-out" => %{module: Module86, struct: %Component{state: %{}}}}, @server}
    end
  end
  ...
  describe "page" do
    ...
    test "interpolate component structs JS" do
      ...
      expected =
        ~s/componentRegistry: Type.map([[Type.bitstring("layout"), Type.map([[Type.atom("module"), Type.atom("Elixir.Hologram.Test.Fixtures.Template.Renderer.Module49")], [Type.atom("struct"), Type.map([[Type.atom("__struct__"), Type.atom("Elixir.Hologram.Component")], [Type.atom("emitted_context"), Type.map([])], [Type.atom("next_action"), Type.atom("nil")], [Type.atom("next_command"), Type.atom("nil")], [Type.atom("next_destroy"), Type.atom("nil")], [Type.atom("next_page"), Type.atom("nil")], [Type.atom("next_warms"), Type.list([])], [Type.atom("state"), Type.map([])]])]])], [Type.bitstring("page"), Type.map([[Type.atom("module"), Type.atom("Elixir.Hologram.Test.Fixtures.Template.Renderer.Module48")], [Type.atom("struct"), Type.map([[Type.atom("__struct__"), Type.atom("Elixir.Hologram.Component")], [Type.atom("emitted_context"), Type.map([[Type.tuple([Type.atom("Elixir.Hologram.Runtime"), Type.atom("csrf_token")]), Type.bitstring("#{@csrf_token}")], [Type.tuple([Type.atom("Elixir.Hologram.Runtime"), Type.atom("initial_page?")]), Type.atom("false")], [Type.tuple([Type.atom("Elixir.Hologram.Runtime"), Type.atom("instance_id")]), Type.bitstring("#{@instance_id}")], [Type.tuple([Type.atom("Elixir.Hologram.Runtime"), Type.atom("page_digest")]), Type.bitstring("102790adb6c3b1956db310be523a7693")], [Type.tuple([Type.atom("Elixir.Hologram.Runtime"), Type.atom("page_mounted?")]), Type.atom("true")]])], [Type.atom("next_action"), Type.atom("nil")], [Type.atom("next_command"), Type.atom("nil")], [Type.atom("next_destroy"), Type.atom("nil")], [Type.atom("next_page"), Type.atom("nil")], [Type.atom("next_warms"), Type.list([])], [Type.atom("state"), Type.map([])]])]])]])/
